...
public class LavenderTown {
    public LavenderTown(Player player) throws SQLException {
        player.location="Lavender Town";
        System.out.println("\n\n\n+----------------------------------------------------------------------+\n" +
                "You are currently in Lavender Town:\n" +
                "+----------------------------------------------------------------------+\n" +
                "[1] Move to:\n" +
                "a.Cerulean City   b.Saffron City   c.Vermillion City   d.Fuchsia city\n" +
                "[2] PokeMaze\n" +
                "[3] Fight Wild Pokemon [Aegislash, Bagon, Banette]\n" +
                "[4] Player Options\n" +
        ...
        if(choice.equals("1a")){
            ...
            while (maze[playerRow][playerCol] != 'E') {
                System.out.print("Enter direction (up(U), down(D), left(L), right(R)): ");
                String direction = scanner.nextLine().toLowerCase();

                if (isValidMove(direction,maze,playerRow,playerCol)) {
                    switch (direction) {
                        case "u":
                            playerRow--;
                            break;
            ...
            if(maze[playerRow][playerCol] == 'E') {
                ...
                for(Pokemon myPokemon:player.pokemons) {
                    myPokemon.xp += 20;
            ...
            Pokemon enemy;
            if(Number==1){
                enemy=createPokemon("Aegislash", myPokemon.level-1);
            }else if(Number==2){
                enemy=createPokemon("Bagon", myPokemon.level-1);
            }else{
                enemy=createPokemon("Banette", myPokemon.level-1);
            }
    ...
    public static Pokemon createLeaderPokemon(String pokemon_type,int pokemon_level) throws SQLException {
        String sql="SELECT * FROM pokemon WHERE Pokemon_type=\""+pokemon_type+"\""+" AND RAND() <= ( SELECT (1/COUNT(*)) FROM pokemon WHERE Pokemon_type='"+pokemon_type+"' ) LIMIT 1";
        String url="jdbc:mysql://localhost:3306/pokemon";
        String username="root";
        String password="Jeeva7396@";
        Connection con= DriverManager.getConnection(url,username,password);
    ...
    private static void displayMaze(char[][] maze,Integer playerRow,Integer playerCol) {
    ...
    private static boolean isValidMove(String direction,char[][] maze,Integer playerRow,Integer playerCol) {
